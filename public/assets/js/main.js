...
function renderResults() {
  for (let i = 0; i < animes.length; i++) {
    ...
    id = anime.mal_id;
    searchResults.innerHTML += `<li class='li-element js-li-element' id='${anime.id}'><img class='anime-img' src='${anime.image_url || 'https://via.placeholder.com/210x295/ffffff/666666/?text=TV'}><h3 class='anime-title'>${anime.title}</h3></li>'`;
}
handlerClickedAnime();
}
...
function renderFavResults() {
  for (let i = 0; i < favAnimes.length; i++) {
    ...
    id = favAnime.mal_id;
    favResults.innerHTML += `<li class='li-element js-li-element fav' id='${favAnime.id}'><img class='anime-img' src='${favAnime.image_url || 'https://via.placeholder.com/210x295/ffffff/666666/?text=TV'}><h3 class='anime-title'>${favAnime.title}</h3></li>'<i class="fas fa-times"></i>`;
}
handlerClickedAnime();
}
